...
MODULE becmod_gpu
  ...
  TYPE bec_type_gpu
     ...
     INTEGER :: mype
     INTEGER :: ibnd_begin
  END TYPE bec_type_gpu
#else
  TYPE bec_type_gpu
     COMPLEX(DP),ALLOCATABLE, DEVICE :: k(:)    ! appropriate for generic k
     INTEGER :: comm
     INTEGER :: nproc
     INTEGER :: mype
     INTEGER :: ibnd_begin
...
#endif
  !
  TYPE (bec_type_gpu) :: becp_gpu  ! <beta|psi>
  ...
  !
  !PUBLIC :: bec_type_gpu, becp_gpu, allocate_bec_type_gpu, deallocate_bec_type_gpu, calbec_gpu, &
  !          beccopy, becscal
  PUBLIC :: bec_type_gpu, becp_gpu, calbec_gpu
  !
CONTAINS
  !-----------------------------------------------------------------------
  SUBROUTINE calbec_bec_type_gpu ( npw, beta, psi, betapsi, ibnd )
    !-----------------------------------------------------------------------
    !_
    ...
    TYPE (bec_type_gpu), INTENT (inout) :: betapsi ! NB: must be INOUT otherwise
    ...
    !
       ...
       !
       CALL  calbec_k_gpu ( npw, beta, psi, betapsi%k, ibnd )
       !
    !
    RETURN
    !
