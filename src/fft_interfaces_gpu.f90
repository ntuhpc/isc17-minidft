...
!
    ...
    !   use fft_scalar,    only: cft_b
      use fft_parallel_gpu,  only: tg_cft3s_gpu
      USE fft_types,     only: fft_dlay_descriptor
...
! !$omp end master 
      ...
      IF( grid_type == 'Dense' ) THEN
         call tg_cft3s_gpu( f, dfftp, 1 )
      ELSE IF( grid_type == 'Smooth' ) THEN
         call tg_cft3s_gpu( f, dffts, 1 )
      ELSE IF( grid_type == 'Wave' ) THEN
         call tg_cft3s_gpu( f, dffts, 2, dffts%have_task_groups )
      ELSE IF( grid_type == 'Custom' ) THEN
         CALL tg_cft3s_gpu( f, dfft, 1 )
      ELSE IF( grid_type == 'CustomWave' ) THEN
